...
Here, ceRNAR comes with 2 experimentally validated miRNA-target databases ([miRTarBase](https://mirtarbase.cuhk.edu.cn/~miRTarBase/miRTarBase_2022/php/index.php) and [miRecords](http://miRecords.umn.edu/miRecords)) and 7 computationally predicted miRNA-target databases ([DIANA-micro T-CDS](https://dianalab.e-ce.uth.gr/html/dianauniverse/index.php?r=microT_CDS), [EIMMO](http://m3rna.cnb.csic.es), [miRDB](https://mirdb.org/), [miRanda](https://bioweb.pasteur.fr/packages/pack@miRanda@3.3a), [PITA](https://genie.weizmann.ac.il/pubs/mir07/mir07_prediction.html), [RNA22](https://cm.jefferson.edu/rna22/) and [TargetScan](https://www.targetscan.org/vert_80/)) for only considering genes with shared miRNAs. Three options for filtering putative miRNA-gene pairs:
...
ceRNApairFilering(path_prefix = '~/', project_name = 'demo', disease_name = "DLBC", window_size = 10)
cernar_result <- SegmentClusteringPlusPeakMerging(path_prefix = '~/', project_name = 'demo', disease_name = "DLBC", window_size = 10)
```
...
function_results <- ceRNAFunction(path_prefix = '~/', project_name = 'demo', disease_name = 'DLBC', pairs_cutoff = 1)
```


```{r message=FALSE, fig.dim = c(20, 30),out.width = "50%"}
function_results[[1]]
```
...
```{r message=FALSE, results='hide'}
location_results <- ceRNALocation(path_prefix = '~/', project_name = 'demo', disease_name = 'DLBC', mirna = 'hsa-miR-101-3p', window_size = 10)
```

```{r echo=FALSE, hmessage=FALSE, fig.dim = c(20, 12)}
location_results
```
...
```{r message=FALSE, results='hide', fig.dim = c(40, 50), out.width = "50%"}
survplot_results <- ceRNASurvival(path_prefix = '~/', project_name = 'demo', disease_name = 'DLBC', mirnas = 'hsa-miR-101-3p')
```
...
```{r message=FALSE, results='hide'}
network_results <- ceRNAModule(path_prefix = '~/', project_name = 'demo', disease_name = 'DLBC', pairs_cutoff = 5, column_sum = 1)
```
...
```{r echo=FALSE}
knitr::kable(external_val_result[1:5,1:5], row.names = F)
```

- **Data integration**

This function allows users to further combine results obtained from ceRNAR with other state-of-the-art tools in ceRNA biological field, including SPONGE [6] and JAMI [7].
...
```{r echo=FALSE, message=FALSE}
knitr::kable(integrated_result[1:5,], row.names = F)
```
...
[6] List M, Dehghani Amirabad A, Kostka D, Schulz MH. Large-scale inference of competing endogenous RNA networks with sparse partial correlation. Bioinformatics. 2019;35(14):i596-i604.

[7] Hornakova A, List M, Vreeken J, Schulz MH. JAMI: fast computation of conditional mutual information for ceRNA network analysis. Bioinformatics. 2018;34(17):3050-1.
